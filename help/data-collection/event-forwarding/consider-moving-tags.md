...
El primer paso es definir los casos de uso implementados con la etiqueta de proveedor del lado del cliente. Por ejemplo, considere el píxel de Facebook (Meta). Cambiarlo de nuestro sitio a la [API de conversiones de Meta](https://exchange.adobe.com/apps/ec/109168/meta-conversions-api) con la extensión de reenvío de eventos significa documentar primero los casos de uso específicos.
...
- ¿Con qué métodos de autenticación son compatibles? ¿Token, HTTP, versión de credenciales del cliente de OAuth u otro? Consulte [aquí](https://experienceleague.adobe.com/es/docs/experience-platform/tags/event-forwarding/secrets) los métodos admitidos por el reenvío de eventos.
