---
title: 'Servicio de consultas: consultas, consultas, consultas... y análisis de pérdida'
description: 'Servicio de consultas: consultas, consultas, consultas... y análisis de pérdida'
kt: 5342
...
# 4.3 Consultas, consultas, consultas... y análisis de pérdida

## Objetivo

* Escribir consultas para análisis de datos
* Escribir consultas SQL que combinen datos en línea, del centro de llamadas y de lealtad disponibles en Adobe Experience Platform
* Obtenga información sobre las funciones definidas por Adobe

## Contexto

En estos ejercicios, escribirá consultas para analizar las vistas de productos, los canales de productos, la reproducción, etc.

Todas las consultas enumeradas en este capítulo se ejecutarán en su **Interfaz de línea de comandos de PSQL**. Debe copiar (CTRL-c) los bloques de instrucciones indicados con **SQL** y péguelos (CTRL-v) en el **Interfaz de línea de comandos de PSQL**. La variable **Resultado de la consulta** los bloques muestran la instrucción SQL pegada y el resultado de consulta asociado.

## 4.3.1 Escribir consultas básicas para el análisis de datos

### Marca de tiempo

Los datos capturados en Adobe Experience Platform llevan una marca de hora. La variable **timestamp** permite analizar los datos a lo largo del tiempo.
...
Copie la instrucción anterior y ejecútela en su **Interfaz de línea de comandos de PSQL**.
...
### Los 5 productos principales vistos
...
### Identificar a los visitantes con riesgo de pérdida (página de visita => Cancelar servicio)
...
En el siguiente conjunto de consultas ampliaremos la consulta anterior, para obtener una vista completa de los clientes y su comportamiento que han estado visitando la página &quot;Cancelar servicio&quot;. Aprenderá a utilizar la función definida por Adobe para clasificar la información, identificar la secuencia y el tiempo de los eventos. También unirá conjuntos de datos para enriquecer aún más y preparar los datos para su análisis en Microsoft Power BI.

## 4.3.2 Consultas avanzadas

La mayoría de la lógica empresarial requiere la recopilación de los puntos de contacto de un cliente y su pedido por tiempo. Spark SQL proporciona esta compatibilidad en forma de funciones de ventana. Las funciones Window son parte de SQL estándar y son compatibles con muchos otros motores SQL.

### Funciones definidas por Adobe

Adobe ha agregado un conjunto de **Funciones definidas por Adobe** a la sintaxis SQL estándar que le permite comprender mejor los datos de experiencia. En el siguiente par de consultas, aprenderá sobre estas funciones de ADF. Puede encontrar más información y la lista completa [en la documentación](https://experienceleague.adobe.com/docs/experience-platform/query/sql/adobe-defined-functions.html).

### ¿Qué hacen las personas en el sitio antes de llegar a la página Cancelar servicio como la tercera página de una sesión?

Con esta consulta descubrirá las dos primeras funciones definidas de Adobe **SESS_TIMEOUT** y **SIGUIENTE**

> La variable **SESS_TIMEOUT()** reproduce las agrupaciones de visitas que se encuentran con Adobe Analytics. Realiza una agrupación similar basada en el tiempo, pero personalizable los parámetros.
>
...
### ¿Cuánto tiempo tenemos antes de que un visitante llame al centro de llamadas después de visitar la página &quot;Cancelar servicio&quot;?

Para responder a este tipo de consulta usaremos la variable **TIME_BETWEEN_NEXT_MATCH()** Función definida por Adobe.

> El tiempo entre las funciones de coincidencia anteriores o siguientes proporciona una nueva dimensión, que mide el tiempo transcurrido desde un incidente en particular.
...
### ¿Y cuál es el resultado de ese contacto?

Explicar que estamos uniendo conjuntos de datos, en este caso nos unimos a nuestra `demo_system_event_dataset_for_website_global_v1_1` con `demo_system_event_dataset_for_call_center_global_v1_1`. Lo hacemos para conocer el resultado de la interacción del centro de llamadas.
...
### ¿Cuál es el perfil de lealtad de estos clientes?

En esta consulta, nos unimos a los datos de fidelidad que hemos incorporado en Adobe Experience Platform. Esto permite enriquecer el análisis de pérdida con datos de lealtad.
...
### ¿De qué región nos visitan?

Incluyamos la información geográfica, como longitud, actitud, ciudad, código de país, capturada por Adobe Experience Platform para obtener información geográfica sobre los clientes que se producen.
...
## Análisis de interacción del centro de llamadas

En las consultas anteriores solo observamos a los visitantes que terminaron contactando con el centro de llamadas en caso de cancelación del servicio. Queremos que esto sea un poco más amplio y tenemos en cuenta toda la interacción del centro de llamadas, incluyendo (WiFi, promoción, factura, queja y contrato).

Tendrá que editar una consulta, así que primero vamos a abrir el bloc de notas o los corchetes.

En Windows, haga clic en el icono &quot;buscar&quot; (1) de la barra de herramientas de Windows, escriba **bloc de notas** en el campo &quot;búsqueda&quot; (2), haga clic (3) en el resultado &quot;bloc de notas&quot;:

![windows-start-notepad.png](./images/windows-start-notepad.png)

En Mac

![osx-start-ackets.png](./images/osx-start-brackets.png)

Copie la siguiente instrucción en bloc de notas/corchetes:
...
No eliminar `/\*` y `\*/`. La instrucción modificada en el bloc de notas debería tener el siguiente aspecto:

![edit-query-notepad.png](./images/edit-query-notepad.png)

Copie la instrucción modificada desde **bloc de notas** en el **Ventana de línea de comandos PSQL** y pulse Intro. Debería ver el siguiente resultado en la ventana de línea de comandos PSQL:
...
En el siguiente período persistirá la consulta (también conocida como **crear tabla al seleccionar** o **CTAS**) como un nuevo conjunto de datos que utilizará en Microsoft Power BI.

Paso siguiente: [4.4 Generar un conjunto de datos a partir de una consulta](./ex4.md)
