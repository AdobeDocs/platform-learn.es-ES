...
En este ejercicio, crearemos áreas de nombres de identidad para los campos de identidad personalizados de Luma: `loyaltyId`, `crmId` y `productSku`. Los espacios de nombres de identidad desempeñan un papel esencial en la creación de perfiles de clientes en tiempo real, ya que dos valores coincidentes en el mismo espacio de nombres permiten que dos fuentes de datos formen un gráfico de identidad.
...
1. Presione el botón **Send** y debería obtener una respuesta de **200 OK**:
   ...
   ![Espacio de nombres de identidad](assets/identity-createUsingApi.png)
...
Con nuestra selección de identidades principales, es claro ver cómo `Luma Loyalty Schema` puede conectarse a `Luma Offline Purchase Events Schema`, ya que ambas utilizan loyaltyId como identificador. Pero, ¿cómo puede conectarse CRM a los eventos de compra sin conexión? ¿Cómo podemos conectar nuestras compras sin conexión con el comportamiento en línea? ¿Y cómo podemos clasificar los productos comprados con nuestro catálogo de productos? Utilizaremos campos de identidad y relaciones de esquema adicionales.
